...
async function compileIfNeeded() {
  ...
  // Copy jqhtml-core.esm.js to output directory for browser testing
  const jqhtmlCoreSrc = path.join(__dirname, '..', 'packages', 'core', 'dist', 'jqhtml-core.esm.js');
  const jqhtmlCoreDest = path.join(OUTPUT_DIR, 'jqhtml-core.esm.js');
  if (fs.existsSync(jqhtmlCoreSrc)) {
    fs.copyFileSync(jqhtmlCoreSrc, jqhtmlCoreDest);
    console.log(`  ✓ Copied jqhtml-core.esm.js to test directory`);
  } else {
    console.warn('  ⚠️  jqhtml-core.esm.js not found, tests may fail');
  }
...
<body>
  ...
  <!-- Track all errors -->
  <script>
    window.jqhtmlTestErrors = [];
    window.addEventListener('error', function(event) {
      console.error('🔥 JavaScript Error:', event.error?.message || event.message);
      if (event.error?.stack) {
        console.error('Stack:', event.error.stack);
      }
      window.jqhtmlTestErrors.push({
  ...
  <!-- Load jQuery (always include for JQHTML templates) -->
  <script src="https://code.jquery.com/jquery-3.7.1.min.js"></script>

  <!-- Load actual JQHTML core runtime (ESM bundle) and THEN load template -->
  <script type="module">
    import jqhtml from './jqhtml-core.esm.js';
    window.jqhtml = jqhtml;
    window.Jqhtml_Component = jqhtml.Jqhtml_Component;

    console.log('✓ JQHTML Core loaded');

    // Now load the compiled template
    await import('./${jsRelativePath}?u=${cacheBuster}');
  </script>
  ...
  <script>
    ...
    window.addEventListener('load', function() {
      setTimeout(function() {
        ...
        if (window.jqhtmlTestErrors.length > 0) {
          console.error('='.repeat(60));
          console.error('ERRORS DETECTED:', window.jqhtmlTestErrors.length);
          console.error('='.repeat(60));
          window.jqhtmlTestErrors.forEach(function(err, idx) {
  ...
  <script>
    // Use actual JQHTML core to instantiate and render components
    setTimeout(function() {
      console.log('='.repeat(60));
      console.log('JQHTML Runtime Ready');
      console.log('='.repeat(60));

      // Create a test container
      const container = $('<div id="test-output">').appendTo('body');

      // Try to instantiate each registered component
      if (window.jqhtml && window.jqhtml._component_registry) {
        const components = Object.keys(window.jqhtml._component_registry);
        console.log('Registered components:', components);

        components.forEach(function(name) {
          console.log('\\nInstantiating component:', name);
          try {
            // Create component instance using JQHTML core
            container.component(name, {});

            console.log('Component rendered successfully');
          } catch (error) {
            console.error('Failed to instantiate component:', error.message);
          }
        });
      }
      ...
      setTimeout(function() {
        console.log('\\n' + '='.repeat(60));
        console.log('FINAL DOM OUTPUT:');
        console.log('='.repeat(60));
        console.log(document.body.innerHTML);
        console.log('\\n' + '='.repeat(60));
        console.log('FORMATTED HTML:');
        console.log('='.repeat(60));
        const formatted = document.body.innerHTML
          .replace(/></g, '>\\n<')
          .split('\\n')
          .map(line => line.trim())
