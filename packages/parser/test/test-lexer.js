...
const test_cases = [
  ...
  <ul>
    ...
    {
        name: "Slot syntax (v2)",
        input: "<#header>Header Content</#header>",
        expected_types: [TokenType.SLOT_START, TokenType.SLOT_NAME, TokenType.GT, TokenType.TEXT, TokenType.SLOT_END, TokenType.SLOT_NAME, TokenType.GT, TokenType.EOF]
    },
    {
        name: "Self-closing slot",
        input: "<#empty />",
        expected_types: [TokenType.SLOT_START, TokenType.SLOT_NAME, TokenType.SLASH, TokenType.GT, TokenType.EOF]
    },
    {
  ...
  <#row>
    <td><%= item.name %></td>
        ...
        expected_contains: ["DataTable", "header", "Name", "row", "item.name"]
    }
];
// Run tests
