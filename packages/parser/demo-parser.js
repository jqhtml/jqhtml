...
const template = `
<Define:BlogPost>
  <article class="blog-post" $id="post_<%= post.id %>">
    <header>
      <h1><%= post.title %></h1>
      <div class="meta">
...
for (const component of ast.body) {
    if (component.type === NodeType.COMPONENT_DEFINITION) {
        ...
        show_component_structure(component, 1);
    }
}
...
// Helper functions
function analyze_ast(node, stats = null) {
    if (!stats) {
        stats = {
            components: [],
            expressions: 0,
    ...
    switch (node.type) {
        ...
        case NodeType.COMPONENT_DEFINITION:
            stats.components.push(node.name);
            for (const child of node.body) {
                analyze_ast(child, stats);
            }
            break;
...
function show_component_structure(node, indent) {
...
function extract_bindings(node, bindings = null) {
    if (!bindings) {
        bindings = {
            expressions: new Set(),
            conditions: new Set(),
    ...
    switch (node.type) {
        ...
        case NodeType.COMPONENT_DEFINITION:
            ...
            break;
        case NodeType.EXPRESSION:
            bindings.expressions.add(node.code);
            break;
        case NodeType.IF_STATEMENT:
            bindings.conditions.add(node.condition);
            for (const child of node.consequent) {
                extract_bindings(child, bindings);
            }
            if (node.alternate) {
            ...
            break;
        case NodeType.FOR_STATEMENT:
            bindings.iterators.add(node.iterator);
            for (const child of node.body) {
                extract_bindings(child, bindings);
            }
            break;
