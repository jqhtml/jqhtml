...
export function process_instructions(instructions: any[], component: any): JQuery {
  const fragment = document.createDocumentFragment();
  const stack: Element[] = [];
  let current: Element | DocumentFragment = fragment;
  ...
  for (const instruction of instructions) {
    ...
    if (instruction.slot) {
      ...
      if (render_fn && typeof render_fn === 'function') {
        ...
        slot_dom.each(function(this: any) {
  ...
  return $(fragment.childNodes as any) as JQuery<HTMLElement>;
}
...
const template_registry = new Map<string, Function>();

// Register a compiled template
export function register_template(name: string, render_fn: Function) {
  template_registry.set(name, render_fn);
}

// Get a compiled template
export function get_template(name: string): Function | undefined {
  return template_registry.get(name);
}

// Create component with template
export function create_templated_component(
  ComponentClass: any,
