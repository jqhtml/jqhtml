...
// Interface for components (matches Component class structure)
...
export function with_template(ComponentClass: any, template_name: string) {
  ...
  ComponentClass.prototype.on_render = async function() {
    ...
    const template_map = (global_obj as any).jqhtml_components;
...
export function register_compiled_templates(template_map: Map<string, any>) {
  // Store on global object for now (MVP approach)
  const global_obj = typeof window !== 'undefined' ? window : global;
  (global_obj as any).jqhtml_components = template_map;
